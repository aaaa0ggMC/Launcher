...
import { getMirrorInfo, switchMirror } from './mirror'
...
function register(name: string, description: string, usage: string | undefined, run: CommandSpec['run']): void {
  commands.set(name, { name, description, usage, run })
}

// -- config ---------------------------------------------------------------
register('config.get', '读取全局配置', 'config.get', async () => readJson(CONFIG_JSON))
register('config.set', '更新全局配置 (--patch <json>)', 'config.set --patch {"theme":"pureblack"}', async (ctx) => {
...
register('apps.get', '读取单个条目 (--root --id)', 'apps.get --root ~/Apps --id bili-viewer', async (ctx) => {
  const root = String(ctx.named.root ?? '')
  const id = String(ctx.named.id ?? '')
  return await getEntry(root, id)
})
register('apps.config', '读取 Apps 能力配置 (搜索目录等)', 'apps.config', async () => getAppsConfig())
register('apps.update', '更新/创建条目 (--root --id --patch <json>)', 'apps.update --root ~/Apps --id bili-viewer --patch {"name":"x"}', async (ctx) => {
  const root = String(ctx.named.root ?? '')
  const id = String(ctx.named.id ?? '')
...
register('apps.delete', '删除条目 (--root --id)', 'apps.delete --root ~/Apps --id start-rdp', async (ctx) => {
  const root = String(ctx.named.root ?? '')
  const id = String(ctx.named.id ?? '')
  if (!root || !id) return { ok: false, error: '需要 --root 与 --id' }
...
register('apps.add-root', '添加搜索目录 (--path)', 'apps.add-root --path /home/aaaa0ggmc/Apps', async (ctx) =>
  addSearchRoot(String(ctx.named.path ?? ''))
)
register('apps.remove-root', '移除搜索目录 (--path)', 'apps.remove-root --path /home/aaaa0ggmc/Apps', async (ctx) =>
  removeSearchRoot(String(ctx.named.path ?? ''))
)
register('apps.rescan', '重扫目录生成草稿 (--root)', 'apps.rescan --root /home/aaaa0ggmc/Apps', async (ctx) => {
  const root = String(ctx.named.root ?? '')
...
// -- launcher ----------------------------------------------------------------
register('launch.run', '启动应用 (--root --id)', 'launch.run --root ~/Apps --id bili-viewer', async (ctx) => {
  const root = String(ctx.named.root ?? '')
  const id = String(ctx.named.id ?? '')
...
register('mirror.switch', '切换镜像源 (--url, pkexec+备份)', 'mirror.switch --url https://mirrors.ustc.edu.cn/archlinux/$repo/os/$arch', async (ctx) => {
  const url = String(ctx.named.url ?? '')
  if (!url) return { ok: false, error: '需要 --url' }
  return await switchMirror(`Server = ${url}`)
})

// -- autostart ---------------------------------------------------------------
register('autostart.list', '列出开机自启动项', 'autostart.list', async () => listAutostart())
register('autostart.toggle', '启用/禁用启动项 (--file --hidden true|false)', 'autostart.toggle --file "Clash Verge.desktop" --hidden true', async (ctx) => {
...
register('systemd.action', '启动/停止/重启服务 (--name --action)', 'systemd.action --name myservice --action restart', async (ctx) => {
...
register('docker.action', '启动/停止/重启容器 (--name --action)', 'docker.action --name new-api --action start', async (ctx) => {
  const name = String(ctx.named.name ?? '')
  const action = String(ctx.named.action ?? '') as 'start' | 'stop' | 'restart'
  if (!name) return { ok: false, error: '需要 --name' }
...
register('hardware.pm', '读取 NVreg_PreserveVideoMemoryAllocations', 'hardware.pm', async () => readPmValue())
register('hardware.pm-toggle', '切换 0↔1 (pkexec, 重启生效)', 'hardware.pm-toggle', async () => togglePm())

// -- display ------------------------------------------------------------------
register('display.wallpapers', '列出壁纸 (--dir)', 'display.wallpapers --dir ~/Pictures/Wallpapers', async (ctx) =>
  listWallpapers(String(ctx.named.dir ?? ''))
)
register('display.apply', '应用壁纸 (--path)', 'display.apply --path /abs/to/wallpaper.jpg', async (ctx) => {
...
export function parseArgs(tokens: string[]): { named: Record<string, string>; positional: string[] } {
...
export async function runCommand(name: string, args: Record<string, unknown> = {}): Promise<unknown> {
  const spec = commands.get(name)
