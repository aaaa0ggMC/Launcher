...
import { forwardGeocodeWithConfig, reverseGeocodeWithConfig } from './geocoding'
import {
  clearTileCache,
...
export async function forwardGeocode(query: string, lang?: string): Promise<GeocodeResult[]> {
  const cfg = await loadYarjConfig()
  return forwardGeocodeWithConfig(query, cfg, lang)
}
...
export async function reverseGeocode(
  ...
  const cfg = await loadYarjConfig()
  return reverseGeocodeWithConfig(lat, lon, cfg, lang)
}
