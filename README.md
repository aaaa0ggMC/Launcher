...
**核心是跨平台的**：Electron 主进程 + Vue 渲染端 + 命令注册表 + 日志管线 + 应用注册表 + 设置/国际化 等基础层不依赖任何发行版或桌面环境。**Windows 已实机验证**（框架 + UI + 应用注册表 + 内置播放器 + 听歌时长统计等纯 Node/Electron 部分均可运行）；macOS 尚未实测，架构上同样可移植。Linux/发行版相关的部分被隔离在各自的 ability service 与 `scripts/` 里，保持命令接口不变即可改写适配（见下「平台适配」）。
...
- 侧栏能力动态加载（Ability 自注入：按 category/name 字母序 + platforms 过滤 + 能力依赖解析）+ 侧栏搜索（应用按名称/别名/标签快速启动，右键上下文菜单可执行注入动作）
- 主题 / 配色方案：10 套内置配色 + 跟随系统（JSON 注册表，可扩展），开启「现代动效」时主题切换带波纹揭示动画，页面切换过渡可选（淡入/滑动/上滑/缩放/翻转）
...
- 听歌时长统计（aidj）：后台按 30s 采样"在听"状态（dbus MPRIS 或内置播放器），每小时一行写入 `~/.config/LinuxCockpit/aidj/time.csv`（`timestamp,duration`，单调递增、直接 append、带时钟回拨保护与重启续写）；查询走 **range(t0,t1) API**——字节级二分定位 + 固定容量 LRU 区间缓存，只读目标区间不全量加载；可视化是**卡片式热度图**（日 5×5 / 月 6×6 / 年 4×4 方形网格，悬浮显示时长与覆盖率），竖向时间线无限滚动、懒加载卸载、粒度记忆；状态栏 chip 与设置页均可一键开关
...
每个能力的 `meta.ts` 声明 `platforms`（适用平台）、`provides`（向系统提供的能力，即能力辞典）与 `dependencies`（要求的能力）。加载器按路径式可满足判定解析，环直接忽略（只是 cmd 依赖）；要求的能力无提供者 → 命令不注册、侧栏不显示并告警。例如 `background` 提供 `background-tasks`，`aidj` / `apps` / `playground` 都依赖它。
