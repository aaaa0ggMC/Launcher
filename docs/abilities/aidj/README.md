...
后台**自动记录你听了多久**——不依赖任何页面打开，应用启动即开始采样。入口：AI DJ 主页菜单 →「听歌时长统计」。

### 4.6.1 判定与记录

- **"在听"判定**：dbus 模式下外部 MPRIS 播放器 `PlaybackStatus == Playing`，web 模式下内置播放器引擎播放中（两种模式互斥，天然去重）。dbus 模式若共享 DBusManager 尚未初始化（从未打开过 AIDJ 页面），会**懒初始化一次**——外部播放器在播、不进入 AIDJ 界面也能被后台计时。
- **采样**：每 30s 一次，播放中 +0.5 分钟；"半分钟以上算一分钟"由落盘时四舍五入保证（一小时最多 120 次采样 → 单行范围 [0,60]）。
- **写盘**：小时翻转时落盘上一小时桶；程序退出时同步补写一次（`process.on('exit')`）。崩溃最多丢当前不足一小时的部分。
- **文件**：`~/.config/LinuxCockpit/aidj/time.csv`，每行 `timestamp,duration`——`timestamp` 为该小时起点（epoch ms，**单调递增 → 直接 append**），`duration` 为该小时听歌分钟数。

### 4.6.2 稳健性设计

- **时钟回拨保护**：写盘时若 `now + 10min < 最后一条 timestamp`（手动改时间/跨时区），丢弃该小时桶不写行，时钟追上来后自动恢复——文件始终保持单调，解析端零修复。
- **重启续写**：上次退出时落盘的部分小时行会在下次启动被续读累计，落盘时**替换**该行而非追加，同一小时永不出现两行。
- **总开关**：状态栏「Listen」chip 或 设置页「听歌时长统计」toggle（`preferences.listening_stats`，默认开）可随时关闭。

### 4.6.3 查询 API（range + 二分 + 缓存）

- `aidj.time-range --start <ms> --end <ms>`：**区间查询**，返回 `{ totalMinutes, rows }`。
- 实现**不全量加载**：内部固定容量 LRU 区间缓存（重复查询零 IO，全量查询不入缓存）；未命中时对单调递增的 timestamp 做**字节级二分**定位上下界（约 log₂(文件/4KB) 次探测），只读目标区间那一小段字节。
- `aidj.time-stats`：全量统计（range API 的薄包装）。

### 4.6.4 热度图界面

- **卡片式热度图**：每个周期（一天 / 一月 / 一年）一张卡片，卡内格子排成方形矩阵（24h → 5×5、一个月 → 6×6、12 个月 → 4×4，边 = `ceil(sqrt(格数))`），一排自适应多张卡片、居中、自动换行。
- **着色**：格子颜色 = `coverage`（该格占满单位时间的比例）——secondary → primary 双色插值 + 透明度加深，跟随当前主题。悬浮显示时长与覆盖率 %。
- **粒度**：日（每天 24h 分布）/ 月（天分布）/ 年（月分布），顶部 combo 切换（选择记忆在 localStorage）。
- **无限滚动**：时间线方向（旧在上、新在下），打开默认定位到当前月份，向上滑无限加载更早、向下滑到"现在"；滑动窗口懒加载 & 卸载（超上限裁掉离视口远的一端）；快速跳转 `YYYY-MM` / 回到当前。
- **每张卡片下方显示该周期总时长**，窗口顶部显示当前加载窗口合计。
